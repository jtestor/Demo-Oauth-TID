...
extension ContentView {
    ...
    func login(){
        Auth0 //provee metodos para la autenticación
            .webAuth() // crea n objeto webAutn que inicializa login universal, requiere el Tenant domain y el app client ID, que se agarra de Auth0
            .start{ result in //metodo del login box
                switch result {
                case .failure (let error):
                    print("Failed with: \(error)")
                case .success (let credentials): // recibe las credenciales y obtiene informacion del usuario
                    self.isAuthenticated = true //cambia el UI a el logged in
